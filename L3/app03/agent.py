...
get_financial_context_agent = Agent(
    name = "get_financial_context_function_executor",
    model = BASEMODEL,
    instruction = "Your only task is to execute the function you have as a tool, when asked to do so",
...
google_search_agent = Agent(
    name = "GoogleSearch",
    model = BASEMODEL,
    instruction = "Your job is to help the agent you are a tool for to search google",
    tools = [google_search]
)
...
root_agent = Agent(
    ...
    *   **Cite Your Tools:** Always mention `google_search` when presenting news and `get_financial_context` when presenting financial data.
    """,
    tools=[agent_tool.AgentTool(google_search_agent), agent_tool.AgentTool(get_financial_context_agent)],
)